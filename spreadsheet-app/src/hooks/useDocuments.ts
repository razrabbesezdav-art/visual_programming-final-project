...
export function useDocuments() {
  ...
  const createDocument = useCallback(async (name: string, rows: number, cols: number) => {
    try {
      const doc = await documentsApi.create({ name, rowCount: rows, colCount: cols })
      await loadDocuments()
      return doc
    } catch (e) {
  ...
  const exportDocument = useCallback(async (id: string, format: 'csv' | 'json') => {
    try {
  ...
  const importDocument = useCallback(async (csvContent: string, name: string) => {
    try {
